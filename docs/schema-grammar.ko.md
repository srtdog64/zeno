...
스키마는 전체 TypeScript가 아니라 `@exornea/zeno-types`의 ABI marker
type을 사용하는 TypeScript 부분집합입니다.
...
의미 alias는 기존 scalar ABI로 내려갑니다. `fixedArray<T, N>`은 `Vector32`
descriptor가 아니라 head 안에 들어가는 inline fixed-layout 영역입니다.
...
`dynamicVector<T>`는 동적 tail 필드를 가진 struct element를 위한 문법입니다.
`Vector32` offset table로 내려가며 `DynamicStructVectorView`를 생성합니다.
Writer helper는 parent arena에서 element head를 예약하고, 중첩 동적 descriptor를
각 element base 기준으로 씁니다.
...
`number`에는 안정적인 ABI width가 없으므로 거부됩니다. `z.i32`, `z.u32`,
...
일반 TypeScript 배열은 descriptor shape와 element layout을 지정하지 않기 때문에
거부됩니다. `z.vector<T>`를 사용해야 합니다.
...
직접 재귀는 inline size가 무한대가 되므로 거부됩니다. `z.pointer<BadNode>`를
사용해야 합니다.
...
optional field는 schema evolution/vtable 정책이 필요하므로 거부됩니다. Zeno는
TypeScript optional 문법을 inline nullable field로 해석하지 않습니다.
...
union은 명시적인 discriminator ABI가 필요하므로 거부됩니다. 향후 지원하려면
...
`.zeno.ts` 파일은 schema-only 파일입니다. 런타임 값 import/export는 거부되며
`@exornea/zeno-types`에서 type-only import만 사용해야 합니다.

## Cross-References

- Construct-to-IR mapping: [layout-ir-coarsening.md](layout-ir-coarsening.md)
- 자세한 시작 가이드: [getting-started.md](getting-started.md)
