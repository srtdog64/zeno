...
export class UserView extends ProjectionView {
  ...
  static assertRecordRange(view: DataView, count: number, baseOffset = 0): void {
    ...
    const requiredByteLength = count * 88;
    if (!Number.isSafeInteger(requiredByteLength)) {
      throw new RangeError(`record range byte length exceeds safe integer: count=${count}, byteLength=88`);
    }
    if (requiredByteLength > view.byteLength - baseOffset) {
      throw new RangeError(`record range exceeds DataView length ${view.byteLength}: baseOffset=${baseOffset}, count=${count}, byteLength=88`);
    }
  }
